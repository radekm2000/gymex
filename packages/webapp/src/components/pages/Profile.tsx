...
import { UserMetricsModal } from "../molecules/modals/UserMetricsModal";

export const Profile = () => {
  const { user, isAuthed } = useAuth();
  const { avatar, discordId } = user.model.discordConnection;

  const username = user.model.user.username;
  const displayName = user.model.user.displayName;
  ...
  return (
    <>
      <UserMetricsModal />
      <div
        className={`flex ${over1200 ? "flex" : "flex-col"} gap-6 mt-4 w-full`}
      >
